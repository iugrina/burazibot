...
# Testing ground for burazi based on the original code from:
...
################################
    ...
    def join_room(self, room, username=None, password=None):
        ...
        self.my_room_JID = '/'.join((room, username))
        pres = xmpp.Presence(to=self.my_room_JID)
    ...
    def callback_message(self, conn, mess):
        ...
        if self.jid.bareMatch(jid):
            return
        # this doesn't work for bot in groups
        # so the following lines are needed
        if type == "groupchat":
            if self.my_room_JID == jid:
                return
